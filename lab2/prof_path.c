...
static inline void func(int cond1, int cond2, int cond3) {
  ...
  // yes this can be done with the algorithm discussed in class, 
  // but we basically already have the solution wiht our edge logic from earlier
  if (e0 && e2 && e1) {
    paths[0]++;
  }
  else if (e0 && e2 && !e1) {
